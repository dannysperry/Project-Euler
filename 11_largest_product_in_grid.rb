...
# 
arr = [%w{08 02 22 97 38 15 00 40 00 75 04 05 07 78 52 12 50 77 91 08},
...
class Array
  def north(x,y)
    y < 3 ? 0 : self[y][x].to_i * self[y-1][x].to_i * self[y-2][x].to_i * self[y-3][x].to_i 
  end

  def northeast(x,y)
    ((y < 3) || (x > self[y].length-4)) ? 0 : self[y][x].to_i * self[y-1][x+1].to_i * self[y-2][x+2].to_i * self[y-3][x+3].to_i 
  end

  def east(x,y)
    x > self[y].length-4 ? 0 : self[y][x].to_i * self[y][x+1].to_i * self[y][x+2].to_i * self[y][x+3].to_i 
  end

  def southeast(x,y)
    ((y > self.length-4) || (x > self[y].length-4)) ? 0 : self[y][x].to_i * self[y+1][x+1].to_i * self[y+2][x+2].to_i * self[y+3][x+3].to_i
  end

  def south(x,y)
    y > self.length-4 ? 0 : self[y][x].to_i * self[y+1][x].to_i * self[y+2][x].to_i * self[y+3][x].to_i 
  end

  def southwest(x,y)
    ((y > self.length-4) || (x < 3)) ? 0 : self[y][x].to_i * self[y+1][x-1].to_i * self[y+2][x-2].to_i * self[y+3][x-3].to_i 
  end

  def west(x,y)
    x < 3 ? 0 : self[y][x].to_i * self[y][x-1].to_i * self[y][x-2].to_i * self[y][x-3].to_i 
  end

  def northwest(x,y)
    ((y < 3) || (x < 3)) ? 0 : self[y][x].to_i * self[y-1][x-1].to_i * self[y-1][x-2].to_i * self[y-1][x-3].to_i 
  end
...
def greatest(array)
  ...
  array.each do |i|
    ...
    i.each do |e|
      x = array[y].index(e)
      result << array.north(x,y) << array.east(x,y) << array.south(x,y) << array.west(x,y) << array.northeast(x,y) << array.southeast(x,y) << array.southwest(x,y) << array.northwest(x,y)
      # 
      # 
    end
  end
